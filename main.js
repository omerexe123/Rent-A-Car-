let menu = document.querySelector('#menu-icon');
let navbar = document.querySelector('.navbar');

menu.onclick = () =>{
    menu.classList.toggle('bx-x');
    navbar.classList.toggle('active');
}

window.onscroll = () => {
    menu.classList.remove('bx-x');
    navbar.classList.remove('active');
}
const sr = ScrollReveal ({
    distance: '60px',
    duration: 2500,
    delay: 400,
    reset: true
})

sr.reveal('.text',({delay: 200, origin: 'top'}))
sr.reveal('.form-container h4',({delay: 600, origin: 'left'}))
sr.reveal('.form-container form',({delay: 600, origin: 'left'}))
sr.reveal('.heading',({delay: 300, origin: 'top'}))
sr.reveal('.ride-container .box' ,({delay: 200, origin: 'top'}))
sr.reveal('.arabalar-container .box',({delay: 300, origin: 'top'}))
sr.reveal('.about-container .box',({delay: 200, origin: 'top'}))
sr.reveal('.yorumlar-container' ,({delay: 200, origin: 'top'}))
sr.reveal('.newsletter .box' ,({delay: 200, origin: 'bottom'}))


function changeImage(imageSrc) {
    document.getElementById('main-image').src = imageSrc;
}


